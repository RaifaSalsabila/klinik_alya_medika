                                ...
                                <td>
                                    <div class="btn-group btn-group-sm">
                                        <button class="btn btn-outline-primary" onclick="viewRecord({{ $record->id }})"
                                                data-bs-toggle="tooltip" title="Lihat Detail Rekam Medis">
                                            <i class="fas fa-eye"></i>
                                        </button>
                                        @if($record->service_type == 'rawat_jalan')
                                        <a href="{{ route('admin.print.prescription', $record->id) }}" class="btn btn-outline-success" target="_blank"
                                           data-bs-toggle="tooltip" title="Cetak Resep Obat">
                                            <i class="fas fa-prescription"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('admin.print.referral', $record->id) }}" class="btn btn-outline-warning" target="_blank"
                                           data-bs-toggle="tooltip" title="Cetak Surat Rujukan">
                                            <i class="fas fa-file-medical-alt"></i>
                                        </a>
                                        @endif
                                        @if($record->invoice)
                                        <a href="{{ route('admin.invoices.show', $record->invoice->id) }}" class="btn btn-success"
                                           data-bs-toggle="tooltip" title="Lihat Invoice - Sudah Dibuat">
                                            <i class="fas fa-file-invoice"></i>
                                        </a>
                                        @else
                                        <a href="{{ route('admin.create-invoice', $record->id) }}" class="btn btn-outline-info"
                                           data-bs-toggle="tooltip" title="Buat Invoice">
...
<script>
    ...
    function viewRecord(recordId) {
        ...
        fetch(`/admin/medical-records/${recordId}`)
    ...
    function printPrescription(recordId) {
        // Open print window for prescription
        window.open(`/admin/medical-records/${recordId}/prescription`, '_blank');
    }

    function printReferral(recordId) {
        // Open print window for referral letter
        window.open(`/admin/medical-records/${recordId}/referral`, '_blank');
    }
</script>
@endsection
